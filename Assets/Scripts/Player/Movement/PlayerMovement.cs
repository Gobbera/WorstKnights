...
public partial class PlayerMovement : MonoBehaviour, IPlayerMovement, IPunObservable
{
    ...
    [Header("Jump State")]
    [SerializeField] [Min(0f)] private float jumpToAirStateDelay = 0.18f;
    [Header("Head Look")]
    ...
    public bool IsKickActionActive => HasAuthority() && IsKickActionActiveAt(Time.time);
    public bool CanStartKick
    {
        get
        {
            if (!HasAuthority())
            ...
            return CanStartKickAt(Time.time, checkCrouchStand: true);
        }
    }
    ...
    public bool CanSprint
    {
        get
        {
            ...
            return HasAvailableStaminaForSprint()
                ...
                && !IsKickActionActiveAt(Time.time)
    ...
    public bool TryAttack()
    {
        ...
        float now = Time.time;
        if (IsKickActionActiveAt(now) || IsNonCombatInventoryActionActive(now))
            return false;
    ...
    private bool IsKickActionActiveAt(float now)
    {
    ...
    private bool CanStartKickAt(float now, bool checkCrouchStand)
    {
        ...
        if (checkCrouchStand && CurrentState == MovementState.crouching && !CanStandFromCrouch())
            return false;

        return HasStaminaForCost(config != null ? config.kickStaminaCost : 0f);
    }
    ...
    public bool TryKick()
    {
        ...
        float now = Time.time;
        if (!CanStartKickAt(now, checkCrouchStand: true))
            return false;

        bool startsFromCrouch = CurrentState == MovementState.crouching;
    ...
    public void Jump()
    {
        ...
        CancelInvoke(nameof(EnterAirStateAfterJump));

        jumpQueued = true;
        jumpAnimationSequence++;
        SetMovementStateInternal(MovementState.jumping);
    ...
    private bool HasStaminaForCost(float amount)
    {
        float staminaCost = Mathf.Max(0f, amount);
        return staminaCost <= StaminaEpsilon || currentStamina + StaminaEpsilon >= staminaCost;
    }
