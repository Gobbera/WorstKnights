...
public partial class PlayerMovement
{
    ...
    private void ScheduleJumpingToAirState()
    {
        CancelInvoke(nameof(EnterAirStateAfterJump));

        float delay = Mathf.Max(0f, jumpToAirStateDelay);
        if (delay <= 0f)
        {
            EnterAirStateAfterJump();
            return;
        }

        Invoke(nameof(EnterAirStateAfterJump), delay);
    }

    private void EnterAirStateAfterJump()
    {
        if (!HasAuthority() || CurrentState != MovementState.jumping)
            return;
