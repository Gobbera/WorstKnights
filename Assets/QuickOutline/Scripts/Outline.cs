...
public class Outline : MonoBehaviour {
  ...
  void Bake() {
    ...
    foreach (var meshFilter in GetComponentsInChildren<MeshFilter>()) {
      ...
      // Non-readable imported meshes cannot expose vertex data in builds.
      if (!CanAccessMeshData(mesh)) {
        continue;
      }
  ...
  void LoadSmoothNormals() {
    ...
    foreach (var meshFilter in GetComponentsInChildren<MeshFilter>()) {
      ...
      // Non-readable imported meshes cannot be modified safely in builds. The
      // fill shader falls back to vertex normals when smooth normals are absent.
      if (!CanAccessMeshData(mesh)) {
        continue;
      }
      ...
      if (HasValidSmoothNormals(mesh, smoothNormals)) {
    ...
    foreach (var skinnedMeshRenderer in GetComponentsInChildren<SkinnedMeshRenderer>()) {
      ...
      // Non-readable imported meshes cannot be modified safely in builds.
      if (!CanAccessMeshData(mesh)) {
        continue;
      }
  ...
  void CombineSubmeshes(Mesh mesh, Material[] materials) {
    ...
    if (!CanAccessMeshData(mesh) || materials == null) {
      return;
    }
  ...
  private static bool CanAccessMeshData(Mesh mesh) {
    return mesh != null && mesh.isReadable;
  }

  private static bool HasValidSmoothNormals(Mesh mesh, List<Vector3> smoothNormals) {
    return mesh != null && smoothNormals != null && smoothNormals.Count == mesh.vertexCount;
  }
