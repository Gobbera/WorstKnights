...
public class PlayerMeleeAttack : MonoBehaviour
{
    ...
    private void ApplyAttackCameraFeedback(float weight)
    {
        ...
        if (hasAttackFeedbackCameraBaseFov && attackFeedbackCamera != null)
            attackFeedbackCamera.fieldOfView = ResolveAttackFeedbackFov(
                attackFeedbackCameraBaseFov,
                hitboxWindow.AttackFovPercentChange,
                safeWeight);

        if (hitboxWindow.ApplyFovFeedbackToHandsCamera && hasAttackFeedbackHandsCameraBaseFov && attackFeedbackHandsCamera != null)
        {
            attackFeedbackHandsCamera.fieldOfView = ResolveAttackFeedbackFov(
                attackFeedbackHandsCameraBaseFov,
                hitboxWindow.AttackFovPercentChange,
                safeWeight);
        }
    ...
    {
        float targetFov = baseFov * (1f + percentChange * 0.01f);
